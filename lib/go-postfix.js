...
export default {
  subscriptions: null,
  keywords: ["apply","map","reduce","print","printf","try","catch","map","error","for","tern","ternary","?","if","must","ife","nn","nil","nnil","nr","ifne","len"],
  exposedKeywords: ["apply","print","printf","map","must","reduce","try","catch","for","tern","?","ife","if","nil","nnil","ifne","len"],
  descriptions: {
    "print": {
      "args": "print",
  ...
  provide(){
    console.log("Calling provide... hm...")
  ...
  expand() {
    ...
    switch (keyword) {
      ...
      case "printf":
        if (argv.length == 0) {
          writeLine(`fmt.Printf("%+v\n", ${ident})`)
        }
        writeLine(`fmt.Printf(${args}, ${ident})`)
        break;
