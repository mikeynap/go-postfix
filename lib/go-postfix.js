...
import GoPostfixView from './go-postfix-view';
...
export default {
  ...
  modalPanel: null,
  subscriptions: null,
  keywords: ["tern", "if","must","ife","nn","nil","nnil","nr","ifne","must2"],

  activate(state) {
    this.goPostfixView = new GoPostfixView(state.goPostfixViewState);
    this.modalPanel = atom.workspace.addModalPanel({
      item: this.goPostfixView.getElement(),
      visible: false
    });
  ...
  deactivate() {
    ...
    this.goPostfixView.destroy();
  },

  serialize() {
    return {
      goPostfixViewState: this.goPostfixView.serialize()
    };
  },
